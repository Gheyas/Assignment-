@extends('back.app')
@section('title', $productCategory->title . ' | Admin - NCG')
@section('heading', $productCategory->title)
@section('content')
@php(session(['url' => url()->current()]))
<div class="row">
    <div class="col-md-12">
        <a href="{{route('admin.product.create', ['product_category_id' => $productCategory->id])}}"
            class="btn btn-outline-success waves-effect float-right width-md waves-light">+ Add new</a>
        <h4 class="m-t-0 header-title">Manage {{$productCategory->title}} Products</h4>
    </div>
</div>
<div class="row pt-2">
